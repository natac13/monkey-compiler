...
func TestResolveGlobal(t *testing.T) {
	...
	for _, sym := range expected {
		...
		if result != sym {
			t.Errorf("expected s=%+v, got=%+v", sym, result)
		}
	}
}
...
func TestResolveLocal(t *testing.T) {
	...
	for _, sym := range expected {
		...
		if result != sym {
			t.Errorf("expected s=%s to resolve to %+v, got=%+v", sym.Name, sym, result)
		}
	}
}
...
func TestResolveNestedLocal(t *testing.T) {
	...
	for _, tt := range tests {
		for _, sym := range tt.expectedSymbols {
			...
			if !ok {
				t.Errorf("name %s not resolvable in table %+v", sym.Name, tt.table)
				continue
			}
			if result != sym {
				t.Errorf("expected s=%s to resolve to %+v, got=%+v", sym.Name, sym, result)
...
func TestResolveFree(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if len(tt.expectedFreeSymbols) != len(tt.table.FreeSymbols) {
			t.Errorf("wrong number of free symbols. want=%d, got=%d", len(tt.expectedFreeSymbols), len(tt.table.FreeSymbols))
			continue
		}
...
func TestResolveUnresolvableFree(t *testing.T) {
	...
	for _, name := range expectedUnresolvable {
		...
		if ok {
			t.Errorf("name %s resolved but expected it to be unresolvable", name)
		}
	}
}
...
func TestDefineAndResolveFunctionName(t *testing.T) {
	...
	if result != expected {
		t.Errorf("expected s=a to resolve to %+v, got=%+v", expected, result)
	}
}
